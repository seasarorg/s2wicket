 ...
 * このアノテーションが付与されたフィールドには，Seasarコンテナにより管理されているコンポーネントがインジェクションされます。<br />
 * <br />
 * このアノテーションは，Wicketが管理するコンポーネントクラスのフィールドに対して付与します。
 * 例えば，WebPageクラスのサブクラス内で以下のように使用します。<br />
 * <pre>
 ...
 * }
 * </pre>
 * 上記のコードでは，Seasarコンテナに"orderService"という名前で登録されているコンポーネントが呼び出し対象となります。<br />
 * <br />
 * Wicketでは，管理下にあるコンポーネントはHTTPセッションに格納されるため，永続される可能性を持っています。
